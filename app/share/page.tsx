...
export default function LocationSharingPage() {
  ...
  const checkLinkValidity = async (linkId: string) => {
    try {
      const invalidatedRef = ref(database, `invalidatedLinks/${linkId}`)
      const invalidatedSnapshot = await get(invalidatedRef)

      if (invalidatedSnapshot.exists()) {
        console.log("[v0] Link is invalidated:", linkId)
        return false
      }

      const linkRef = ref(database, `generatedLinks/${linkId}`)
      const linkSnapshot = await get(linkRef)

      if (!linkSnapshot.exists()) {
        console.log("[v0] Link not found in generated links:", linkId)
        return false
      }

      console.log("[v0] Link is valid:", linkId)
      return true
    } catch (error) {
    ...
    try {
      ...
      if (!database) {
        ...
        toast({
          ...
          description: "Таны байршил хадгалагдлаа.",
        })
        setIsLoading(false)
        return
      }
    ...
    try {
      if (window.google && window.google.maps && window.google.maps.places) {
        ...
        service.getPlacePredictions(
          ...
          (predictions: GooglePlacePrediction[] | null, status: PlacesServiceStatus) => {
    ...
    try {
      if (window.google && window.google.maps && window.google.maps.places) {
        ...
        service.getDetails(
          ...
          (place: GooglePlaceDetails | null, status: PlacesServiceStatus) => {
  ...
  const sendSelectedLocation = async () => {
    ...
    try {
      ...
      if (!database) {
        ...
        toast({
          title: "Байршил амжилттай илгээгдлээ",
          description: "Сонгосон байршил хадгалагдлаа.",
        })
        setIsLoading(false)
        return
      }
  ...
  useEffect(() => {
    ...
    const validateLink = async () => {
      ...
      if (currentLinkId && currentLinkId !== "share") {
        const isLinkStillValid = await checkLinkValidity(currentLinkId)

        if (!isLinkStillValid) {
          toast({
            title: "Линк хүчингүй",
            description: "Энэ линк хүчингүй болсон эсвэл холбогдох байршил устгагдсан байна.",
            variant: "destructive",
          })
          setTimeout(() => {
